...
if (isset($_GET['pdf'])) {
	...
	foreach ($list_candidatos as $candidato) {
		...
		//Obtenemos si la imagen es horinzontal o vertical si no esta dejar un espacio en blanco

		// Validamos que exista la firma y no esté vacía
		if (!empty($candidato["firma"])) {

			// Decodificamos el base64
			$imgData = base64_decode($candidato["firma"]);

			// Obtenemos tamaño de la imagen desde string
			...
			// Si no es imagen válida, simplemente no se imprime nada
		}
	...
	if (isset($_SESSION['array_candidatos'])) {
		...
		try {
			...
			$sheet->getStyle('A5:L5')->getFill()->setFillType("solid")->getStartColor()->setARGB('FFE5E5E5');
			...
			$sheet->setCellValue('F5', 'L.NACIM.');
			$sheet->setCellValue('G5', 'PADRE');
			$sheet->setCellValue('H5', 'MADRE');
			$sheet->setCellValue('I5', 'E. CIVIL');
			$sheet->setCellValue('J5', 'SEXO');
			$sheet->setCellValue('K5', 'NACION.');
			$sheet->setCellValue('L5', 'NIE');
			$sheet->getStyle('A5:L5')->getAlignment()->setHorizontal('center');
			$sheet->getStyle('A5:L5')->getBorders()->getAllBorders()->setBorderStyle("thin");
			...
			foreach ($_SESSION['array_candidatos'] as $candidato) {
				...
				$sheet->setCellValue('F' . $i, $candidato['lugar_nac']);
				$sheet->setCellValue('G' . $i, $candidato['nombre_padre']);
				$sheet->setCellValue('H' . $i, $candidato['nombre_madre']);
				if ($candidato['estado_civil'] == 0) {
					//Estado Civil S
					$sheet->setCellValue('I' . $i, 'Soltero/a');
				} elseif ($candidato['estado_civil'] == 1) {
					//Estado Civil C
					$sheet->setCellValue('I' . $i, 'Casado/a');
				} elseif ($candidato['estado_civil'] == 2) {
					//Estado Civil V
					$sheet->setCellValue('I' . $i, 'Viudo/a');
				} elseif ($candidato['estado_civil'] == 3) {
					//Estado Civil D
					$sheet->setCellValue('I' . $i, 'Divorciado/a');
				} elseif ($candidato['estado_civil'] == 4) {
					//Estado Civil Sp
					$sheet->setCellValue('I' . $i, 'Separado/a');
				}
				$sheet->setCellValue('J' . $i, $candidato['sexo']);
				$sheet->setCellValue('K' . $i, $candidato['nacionalidad']);
				$sheet->setCellValue('L' . $i, '');
				$sheet->getStyle('A' . $i . ':L' . $i)->getAlignment()->setHorizontal('center');
				$sheet->getStyle('A' . $i . ':L' . $i)->getBorders()->getAllBorders()->setBorderStyle("thin");
				$i++;
				$num++;
			}
...
} elseif (isset($_GET['informe_presencia_ofi_audi_pdf_new'])) {
	// Creación del objeto de la clase heredada
	$pdf = new tFPDF('L');
	$pdf->SetMargins(26, 20, 20); // Margen de la tabla
	// $pdf->SetAutoPageBreak(true, 20);
	...
	// Función para mostrar encabezado de trabajador
	function headerTrabajador($pdf, $pernr, $nombre)
	{
		$pdf->SetFont('Arial', 'B', 10);
		$pdf->SetFillColor(200, 220, 255);
		$pdf->Cell(245, 6, utf8_decode('Cod. Trab: ' . $pernr . ' - ' . $nombre), 1, 1, 'L', true);
	}

	// Cabecera de la tabla de registros (sin Cod. Trab y Nombre)
	function headerTabla($pdf)
	{
		$pdf->SetFont('Arial', 'B', 8);
		$pdf->SetFillColor(222, 222, 222);
		$pdf->Cell(25, 6, utf8_decode('Fecha'), 1, 0, 'C', true);
		$pdf->Cell(30, 6, utf8_decode('Primera Entrada'), 1, 0, 'C', true);
		$pdf->Cell(30, 6, utf8_decode('Última Salida'), 1, 0, 'C', true);
		$pdf->Cell(20, 6, utf8_decode('Desayuno'), 1, 0, 'C', true);
		$pdf->Cell(20, 6, utf8_decode('Almuerzo'), 1, 0, 'C', true);
		$pdf->Cell(18, 6, utf8_decode('Otros'), 1, 0, 'C', true);
		$pdf->Cell(35, 6, utf8_decode('Descanso Total'), 1, 0, 'C', true);
		$pdf->Cell(35, 6, utf8_decode('Horas trabajadas'), 1, 0, 'C', true);
		$pdf->Cell(32, 6, utf8_decode('Horas Totales'), 1, 1, 'C', true);
	}

	// Función para convertir DateTime a formato HH:mm
	...
	// Inicializar variables para sumar los segundos de los horarios
	$segundos_desayuno = 0;
	$segundos_almuerzo = 0;
	$segundos_otros = 0;
	$segundos_descanso = 0;
	...
	if (!empty($datosInforme)) {
		// Agrupar datos por trabajador (pernr)
		$datosPorTrabajador = [];
		foreach ($datosInforme as $resultado) {
			$pernr = $resultado['pernr'];
			if (!isset($datosPorTrabajador[$pernr])) {
				$datosPorTrabajador[$pernr] = [
					'nombre' => ($resultado['NOMBREYAPELLIDOS'] != '') ? $resultado['NOMBREYAPELLIDOS'] : 'Desconocido',
					'registros' => []
				];
			}
			$datosPorTrabajador[$pernr]['registros'][] = $resultado;
		}

		// Iterar por cada trabajador y sus registros
		foreach ($datosPorTrabajador as $pernr => $datosTrabajador) {
			// Mostrar encabezado del trabajador
			headerTrabajador($pdf, $pernr, $datosTrabajador['nombre']);

			// Mostrar cabecera de la tabla de registros
			headerTabla($pdf);

			// Mostrar los registros del trabajador
			foreach ($datosTrabajador['registros'] as $resultado) {
				$segundos_desayuno += $resultado['segundos_desayuno'];
				$segundos_almuerzo += $resultado['segundos_almuerzo'];
				$segundos_otros += $resultado['segundos_otros'];
				$segundos_descanso += $resultado['segundos_descanso'];
				$segundos_tiempo_efectivo += $resultado['segundos_producido'];
				$segundos_horas_totales += $resultado['segundos_totales'];

				$pdf->SetFont('Arial', '', 8);

				// Fecha
				if (isset($resultado['fecha']) && $resultado['fecha'] instanceof DateTime) {
					$pdf->Cell(25, 6, $resultado['fecha']->format('Y/m/d'), 1, 0, 'C');
				} else {
					$pdf->Cell(25, 6, 'N/A', 1, 0, 'C');
				}

				// Primera Entrada
				$pdf->Cell(30, 6, formatearHora($resultado['primera_entrada']), 1, 0, 'C');
				// Última Salida
				$pdf->Cell(30, 6, formatearHora($resultado['ultima_salida']), 1, 0, 'C');

				// Resto de campos
				$pdf->Cell(20, 6, utf8_decode($resultado['horas_desayuno']), 1, 0, 'C');
				$pdf->Cell(20, 6, utf8_decode($resultado['horas_almuerzo']), 1, 0, 'C');
				$pdf->Cell(18, 6, utf8_decode($resultado['horas_otros']), 1, 0, 'C');
				$pdf->Cell(35, 6, utf8_decode($resultado['horas_descanso']), 1, 0, 'C');
				$pdf->Cell(35, 6, utf8_decode($resultado['horas_producido']), 1, 0, 'C');
				$pdf->Cell(32, 6, utf8_decode($resultado['horas_totales']), 1, 1, 'C');
				...
				if ($pdf->GetY() > 180) {
					...
					// NO repetir encabezados, continuar directamente con los registros
				}
			}
		...
		$pdf->SetFont('Arial', 'B', 9);
		$pdf->Ln(2);
		$pdf->Cell(70, 6, utf8_decode('TOTALES GENERALES'), 0, 1, 'L');

		$pdf->Cell(85, 6, utf8_decode(''), 0, 0, '');
		$pdf->Cell(20, 6, utf8_decode('Desayuno'), 1, 0, 'C', true);
		$pdf->Cell(20, 6, utf8_decode('Almuerzo'), 1, 0, 'C', true);
		$pdf->Cell(18, 6, utf8_decode('Otros'), 1, 0, 'C', true);
		$pdf->Cell(35, 6, utf8_decode('Descanso Total'), 1, 0, 'C', true);
		...
		$pdf->Cell(20, 6, convertir_a_horas($segundos_almuerzo), 1, 0, 'C');
		$pdf->Cell(18, 6, convertir_a_horas($segundos_otros), 1, 0, 'C');
		$pdf->Cell(35, 6, convertir_a_horas($segundos_descanso), 1, 0, 'C');
		$pdf->Cell(35, 6, convertir_a_horas($segundos_tiempo_efectivo), 1, 0, 'C');
		$pdf->Cell(32, 6, convertir_a_horas($segundos_horas_totales), 1, 0, 'C');
	}
	...
	try {
		...
		$sheet1->setTitle("INFORME PRESENCIA OFICINA");
	...
	if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_GET['etiqueta_nfc'])) {
		...
		$pdf = new tFPDF('L', 'mm', [57, 32]); // Tamaño 57x32 mm, orientación Landscape
		$pdf->AddPage();
